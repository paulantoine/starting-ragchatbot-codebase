...
class TestRAGSystemIntegration:
    ...
    def test_add_course_document_success(self, mock_exists, rag_system, sample_course, sample_course_chunks):
        """Test successful course document addition"""
        # Setup
        mock_exists.return_value = True
        rag_system.document_processor.process_course_document.return_value = (sample_course, sample_course_chunks)
    ...
    def test_add_course_folder_success(self, mock_isfile, mock_exists, mock_listdir, rag_system, sample_course, sample_course_chunks):
        ...
        course1 = Course(title="Course 1", course_link="https://example.com/course1", instructor="Instructor 1")
        course2 = Course(title="Course 2", course_link="https://example.com/course2", instructor="Instructor 2")
        chunks1 = [CourseChunk(content="Course 1 content", course_title="Course 1", lesson_number=0, chunk_index=0)]
        chunks2 = [CourseChunk(content="Course 2 content", course_title="Course 2", lesson_number=0, chunk_index=0)]
    ...
    @patch('os.listdir')
    @patch('os.path.exists')
    def test_add_course_folder_skip_existing_courses(self, mock_exists, mock_listdir, rag_system, sample_course, sample_course_chunks):
    ...
    def test_query_without_session(self, rag_system):
        ...
        rag_system.tool_manager.get_last_sources.return_value = [{"text": "Test Course", "link": None}]
    ...
    def test_query_with_session(self, rag_system):
        ...
        rag_system.session_manager.get_conversation_history.return_value = conversation_history
        rag_system.ai_generator.generate_response.return_value = "Context-aware response"
    ...
    def test_query_with_tool_usage(self, rag_system):
        ...
        rag_system.tool_manager.get_last_sources.return_value = [
            ...
            {"text": "Another Course - Lesson 2", "link": "https://example.com/lesson/2"}
        ]
    ...
    def test_query_course_specific_content(self, rag_system):
        ...
        rag_system.ai_generator.generate_response.return_value = "Course-specific response"
        rag_system.tool_manager.get_last_sources.return_value = [{"text": "Advanced Course", "link": None}]
    ...
    def test_end_to_end_workflow_with_mocked_components(self, mock_config, sample_course, sample_course_chunks):
        """Test complete end-to-end workflow with carefully mocked components"""
        with patch('rag_system.DocumentProcessor') as mock_doc_proc_class, \
            ...
            # Setup workflow
            mock_doc_proc.process_course_document.return_value = (sample_course, sample_course_chunks)
            ...
            mock_tool_mgr.get_last_sources.return_value = [{"text": "Test Course", "link": None}]
    ...
    def test_error_handling_in_query_processing(self, rag_system):
        ...
        rag_system.ai_generator.generate_response.side_effect = Exception("AI API error")
    ...
    def test_session_management_integration(self, rag_system):
        ...
        rag_system.session_manager.get_conversation_history.return_value = conversation_history
        rag_system.ai_generator.generate_response.return_value = "Second response with context"
        
        response2, _ = rag_system.query("Follow up", session_id=session_id)
        
        # Verify history was used
        second_call_args = rag_system.ai_generator.generate_response.call_args_list[1][1]
