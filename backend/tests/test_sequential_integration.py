...
"""
                ...
                if "neural networks" in query.lower() or "deep learning" in query.lower():
...
"""
    ...
    def test_complex_course_comparison_scenario(self, mock_anthropic_class, mock_tool_manager):
        """
        ...
        final_response.content[0].text = """Based on the course outline, lesson 4 of Machine Learning Fundamentals covers "Neural Networks and Deep Learning". 
...
Advanced Deep Learning would be the most similar course as it focuses specifically on the deep learning concepts covered in lesson 4."""
    ...
    @patch('anthropic.Anthropic')
    def test_multi_course_comparison_scenario(self, mock_anthropic_class, mock_tool_manager):
        """
        ...
        final_response = Mock()
        final_response.content = [Mock()]
        final_response.content[0].text = "Comprehensive comparison of machine learning courses based on search and outline results."
        ...
        assert result == "Comprehensive comparison of machine learning courses based on search and outline results."

    @patch('anthropic.Anthropic')
    def test_progressive_refinement_scenario(self, mock_anthropic_class, mock_tool_manager):
        """
        ...
        second_response.content[0].text = "Based on the search, here are the available data science courses with their key topics."
        ...
        assert mock_client.messages.create.call_count == 2  # Stopped after first round + response
